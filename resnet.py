...
from resnet_v1 import bottleneck, bottleneck_skip, sub_pixel, sub_pixel_skip, noconcat,attention,attention_layer,tail_att
...
class ResNet(object):
    ...
    def add_extra_layers(self, net):
        with slim.arg_scope(resnet_v1.resnet_arg_scope(is_training=self.training,
            ...
            blocks = [
                resnet_utils.Block(
                    'block5', bottleneck, [(num_fm//2, num_fm//2, 2)] + [(num_fm//2, num_fm//2, 1)] * (block_depth - 1)),
                resnet_utils.Block(
                    'block6', bottleneck, [(num_fm//2, num_fm//2, 2)] + [(num_fm//2, num_fm//2, 1)] * (block_depth - 1)),
                resnet_utils.Block(
                    'block7', bottleneck, [(num_fm//2, num_fm//2, 1)] + [(num_fm//2, num_fm//2, 1)] * (block_depth - 1)),
            ]
            ...
            with tf.variable_scope(DEFAULT_SSD_SCOPE+"_back", reuse=self.reuse):
                ...
                with slim.arg_scope([slim.conv2d, tail_att],
                    ...
                    if args.image_size == 512:
                        ...
                        #                      top_fm, int_fm, scope='block_rev7')
                        net = tail_att(net, self.outputs[DEFAULT_SSD_SCOPE + '/block7'],
                                              top_fm, int_fm, scope='block_rev7')
                    '''''
                    ...
                    '''
                    net = tail_att(net, self.outputs[DEFAULT_SSD_SCOPE + '/block6'],
                                   top_fm, top_fm, scope='block_rev6')
                    net = tail_att(net, self.outputs[DEFAULT_SSD_SCOPE + '/block5'],
                                   top_fm, top_fm, scope='block_rev5')
                    net = tail_att(net, self.outputs[self.scope + '/block4'],
                                   top_fm, top_fm, scope='block_rev4')
                    net = tail_att(net, self.outputs[self.scope + '/block3'],
                                   top_fm, top_fm, scope='block_rev3')
                    net = tail_att(net, self.outputs[self.scope + '/block2'],
                                   top_fm, top_fm, scope='block_rev2')
                    if args.x4:
                        ...
                        #                      top_fm, int_fm, scope='block_rev1')
                        net = tail_att(net, self.outputs[self.scope + '/block1'],
                                              top_fm, int_fm, scope='block_rev1')
    ...
    def create_multibox_head(self, num_classes):
        ...
        with tf.variable_scope(DEFAULT_SSD_SCOPE, reuse=self.reuse) as sc:
            ...
            with slim.arg_scope(self.vgg_arg_scope()):
                with slim.arg_scope([slim.conv2d], outputs_collections=end_points_collection,
                                    ...
                                    activation_fn=None):
                    for i, layer_name in enumerate(self.layers):        # for把每一层输出按要求改变尺寸之后append进对应的数组
                        ...
                        loc = slim.conv2d(src_layer, num_priors * 4,             # num_priors * 4 = 64,输出tensor深度为64
                                          [args.det_kernel, args.det_kernel],    # 3*3
                                          scope='location'+scope_suffix)
                        loc_sh = tf.stack([batch_size, wh * num_priors, 4])
                        locations.append(tf.reshape(loc, loc_sh))                # 把每一层的输出都放进去
        ...
        all_confidences = tf.concat(confidences, 1)                               # 最终的预测模块
        ...
        with tf.variable_scope(DEFAULT_SSD_SCOPE) as sc:
            with slim.arg_scope([slim.conv2d],
                ...
                for i in range(len(self.layers)):
                    ...
                    seg = slim.conv2d(target_layer, args.n_base_channels)    # 深度args.n_base_channels为64
                    seg = tf.image.resize_nearest_neighbor(seg, [seg_size, seg_size])     # 尺寸为37 * 37
                ...
                '''********************attention****************************'''
                #seg_logits = attention_block(seg_logits)
                '''*********************************************************'''
