...
def train(dataset, net, config):
    ...
    net.create_trunk(image_ph)                # 构建只包含Encoder-Decoder的架构

    if args.detect:                           # 需要检测功能则在上面的架构中添加multibox_head
    ...
    if args.segment:                          # 需要分割功能则在上面的架构中添加segmentation_head
    ...
    learning_rate = tf.train.piecewise_constant(tf.to_int32(global_step),    # 衰减的学习率
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    if args.dataset == 'voc12-train':
        ...
        #dataset = get_dataset('voc12-training-seg-aug')
    ...
    if args.dataset == 'voc07+12':
        ...
        dataset = get_dataset('voc07_trainval', 'voc12-train')
        #dataset = get_dataset('voc07_trainval', 'voc12_trainval')
    ...
    if args.dataset == 'voc07+12-segmentation':
        #dataset = get_dataset('voc07-trainval-segmentation', 'voc12-train-segmentation')
        #dataset = get_dataset('voc12-val-seg', 'voc12-training-seg-aug')
        dataset = get_dataset('voc12-training-seg-aug')
    ...
    if args.dataset == 'coco-seg':
        # support by default for coco trainval35k split
        dataset = get_dataset('coco-seg-train2014-*', 'coco-seg-valminusminival201'
                                                      '4-*')
...
if __name__ == '__main__':
    ...
    git_diff = subprocess.check_output('git diff --no-color'.split()).decode('utf-8')
